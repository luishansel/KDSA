...
else
{
	class PDF extends TCPDF
	{
		...
		function Header()
		{
			...
			$msHTML = '<table>';
			$msHTML .= '<tr>';
			$msHTML .= '<th style="text-align: left;background-color:rgb(0,0,255);color: white; width: 30%;"><strong>Curso</strong></th>';
			$msHTML .= '<th style="text-align: left;background-color:rgb(0,0,255);color: white; width: 15%;"><strong>Docente</strong></th>';
			$msHTML .= '<th style="text-align: center;background-color:rgb(0,0,255);color: white; width: 15%;"><strong>Período de clases</strong></th>';
			$msHTML .= '<th style="text-align: center;background-color:rgb(0,0,255);color: white; width: 8%;"><strong>Inicial</strong></th>';
			$msHTML .= '<th style="text-align: center;background-color:rgb(0,0,255);color: white; width: 8%;"><strong>Activo</strong></th>';
			$msHTML .= '<th style="text-align: center;background-color:rgb(0,0,255);color: white; width: 8%;"><strong>Deserción</strong></th>';
			$msHTML .= '<th style="text-align: center;background-color:rgb(0,0,255);color: white; width: 8%;"><strong>Baja</strong></th>';
	...
	if ($mnTipoRep == 0)
	{
		$msConsulta = "select CURSO_REL, NOMBRE_020, FECHAINI_020, FECHAFIN_020, concat(CONVOCATORIA_020, ' / ', 'G', GRUPO_020) as CONVOCATORIA from KDSA020A where ACTIVO_020 = 1 and FECHAINI_020 between ? and ?";
		$msPeriodo = "Cursos activos que iniciaron entre " . DevuelveFecha($mdFechaIni) . " y " . DevuelveFecha($mdFechaFin);
	}
	else
	{
		$msConsulta = "select CURSO_REL, NOMBRE_020, FECHAINI_020, FECHAFIN_020, concat(CONVOCATORIA_020, ' / ', 'G', GRUPO_020) as CONVOCATORIA from KDSA020A where ACTIVO_020 = 1 and FECHAFIN_020 between ? and ?";
		$msPeriodo = "Cursos activos que finalizaron entre " . DevuelveFecha($mdFechaIni) . " y " . DevuelveFecha($mdFechaFin);
	}
	...
	while ($Fila = $mDatos->fetch())
	{
		...
		$msConsulta .= "select  5 as ESTADO, COUNT(MATRICULA_REL) as CONTEO from KDSA030A where ESTADO_030 = 5 and CURSO_REL = ?";
		$mAux = $m_cnx_MySQL->prepare($msConsulta);
		$mAux->execute([$msCurso, $msCurso, $msCurso, $msCurso]);
		...
		if ($mnActivo > 0)
			$mnRetencion = ($mnActivo * 100) / $mnInicial;
		else
			$mnRetencion = 0;
		...
		if ($mbColorea == 0)
		{
			$msHTML .= '<td style="text-align: left; width: 30%;">' . trim($msNomCurso) . ' (' . trim($msConvocatoria) . ')</td>';
			$msHTML .= '<td style="text-align: left; width: 15%;">' . trim($msDocente) . '</td>';
			$msHTML .= '<td style="text-align: center; width: 15%;">' . trim($msPeriodo) . '</td>';
			...
			$msHTML .= '<td style="text-align: center; width: 8%;">' . trim($mnBaja) . '</td>';
			$msHTML .= '<td style="text-align: center; width: 8%;">' . number_format($mnRetencion, 2) . '</td>';
		}
		else
		{
			$msHTML .= '<td style="background-color: #f2f2f2; text-align: left; width: 30%;">' . trim($msNomCurso) . ' (' . trim($msConvocatoria) . ')</td>';
			$msHTML .= '<td style="background-color: #f2f2f2; text-align: left; width: 15%;">' . trim($msDocente) . '</td>';
			$msHTML .= '<td style="background-color: #f2f2f2; text-align: center; width: 15%;">' . trim($msPeriodo) . '</td>';
			...
			$msHTML .= '<td style="background-color: #f2f2f2; text-align: center; width: 8%;">' . trim($mnBaja) . '</td>';
	...
	if ($mnTotalInicial == 0)
		...
		$mnPromedioRetencion = ($mnTotalActivo / $mnTotalInicial) * 100;
	
	$msHTML .= '<tr nobr="true">';
	$msHTML .= '<td style="text-align: left;background-color:rgb(0,0,255);color: white; width: 30%;">&nbsp;</td>';
	$msHTML .= '<td style="text-align: left;background-color:rgb(0,0,255);color: white; width: 15%;">&nbsp;</td>';
	$msHTML .= '<td style="text-align: center;background-color:rgb(0,0,255);color: white; width: 15%;">&nbsp;</td>';
	...
	$msHTML .= '<td style="text-align: center;background-color:rgb(0,0,255);color: white; width: 8%;"><strong>' . trim($mnTotalBaja) . '</strong></td>';
