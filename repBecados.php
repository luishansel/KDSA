...
else
{
	class PDF extends TCPDF
	{
		...
		function Header()
		{
			...
			$msHTML .= '<th style="text-align: left;background-color:rgb(0,0,255);color: white; width: 30%;"><strong>Becado por</strong></th>';
	...
	$msConsulta = "select FECHA_030, BECADOPOR_030, INATEC_030, NOMBRES_010, APELLIDOS_010, TIPO_020, NOMBRE_020, CONVOCATORIA_020, GRUPO_020 ";
	$msConsulta .= "from KDSA030A, KDSA020A, KDSA010A ";
	$msConsulta .= "where BECADO_030 = 1 and KDSA030A.CURSO_REL = KDSA020A.CURSO_REL and KDSA030A.ESTUDIANTE_REL = KDSA010A.ESTUDIANTE_REL and FECHA_030 between ? and ?";
	...
	while ($Fila = $mDatos->fetch())
	{
		...
		$msBecadoPor = html_entity_decode($Fila["BECADOPOR_030"]);
		if (intval($Fila["INATEC_030"]) == 1)
			$msBecadoPor .= " / INATEC";
		...
		if ($mbColorea == 0)
		{
			...
			$msHTML .= '<td style="text-align: left; width: 30%;">' . trim($msBecadoPor) . '</td>';
		...
		else
		{
			...
			$msHTML .= '<td style="background-color: #f2f2f2; text-align: left; width: 30%;">' . trim($msBecadoPor) . '</td>';
